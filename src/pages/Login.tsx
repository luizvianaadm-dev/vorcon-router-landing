...
import { KeyRound, Shield, AlertCircle, Zap, Eye, EyeOff } from 'lucide-react';
...
export default function Login() {
  ...
  return (
    <div className="min-h-screen relative font-sans text-slate-200 flex items-center justify-center p-4 overflow-hidden bg-black">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8 }} className="w-full max-w-md relative z-10">
        <div className="glass-panel rounded-2xl overflow-hidden shadow-2xl border border-slate-700/50 backdrop-blur-xl bg-[#0A0F1C]/60">
          ...
          <div className="p-10">
            ...
            <div className="flex justify-center mb-8">
              {!isAuraMode ? (
                <div onClick={handleShieldClick} className="w-20 h-20 rounded-2xl bg-slate-900 border border-brand-whatsapp/40 flex items-center justify-center shadow-[0_0_30px_rgba(37,211,102,0.3)] transform -rotate-6 hover:rotate-0 transition-all duration-500 cursor-pointer">
                  <KeyRound className="w-10 h-10 text-brand-whatsapp" />
                </div>
              ) : (
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-24 h-24 rounded-full bg-slate-900 border border-brand-glow flex items-center justify-center shadow-[0_0_50px_rgba(37,211,102,0.5)]">
            ...
            <p className="text-center text-slate-400 text-sm mb-8">
              {isAuraMode ? 'Aguardando frequência ultrassônica da chave AURA...' : 'Autenticação segura para clientes.'}
            </p>
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Link to="/reset-password" className="text-xs text-brand-whatsapp/80 hover:text-brand-whatsapp transition-colors">Esqueceu a senha?</Link>
                  </div>
